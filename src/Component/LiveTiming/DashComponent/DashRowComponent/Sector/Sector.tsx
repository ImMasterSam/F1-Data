...
function Sector({ sector }: Props) {
  return <div className="sector-feild">
    ...
    <div className="sector-time">
      <h4 className={`${sector.sectorLast.sectorTime != '-- ---' ? '' : 'previous'}`} style={{color: `${lastSector_color(sector.sectorLast)}`}}>{sector.sectorLast.sectorTime != '-- ---' ? sector.sectorLast.sectorTime : sector.sectorLast.previousSectorTime}</h4>
